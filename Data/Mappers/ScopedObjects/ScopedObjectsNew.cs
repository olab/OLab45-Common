...
{
  public class ScopedObjectsNew : ObjectMapper<Model.ScopedObjects, ScopedObjectsNewDto>
  {
    protected readonly bool enableWikiTranslation = true;

    public ScopedObjectsNew(OLabLogger logger, bool enableWikiTranslation = true) : base(logger)
    {
    }

    public ScopedObjectsNew(OLabLogger logger, WikiTagProvider tagProvider, bool enableWikiTranslation = true) : base(logger, tagProvider)
    {
    }

    public override ScopedObjectsNewDto PhysicalToDto(Model.ScopedObjects phys, object source = null)
    {
      ScopedObjectsNewDto dto = GetDto(source);

      foreach (var question in phys.Questions)
      {
        var questionDto = question.Adapt<QuestionsFullNewDto>(QuestionsFullNewDto.GetMapsterConfig());
        dto.Questions.Add(questionDto);
      }

      //var dtoQuestionsList = new ObjectMapper.QuestionsFull(logger, GetWikiProvider()).PhysicalToDto(phys.Questions);
  ...
  = new ObjectMapper.Counters(logger, GetWikiProvider()).PhysicalToDto(phys.Counters);
    ...
    public override Model.ScopedObjects DtoToPhysical(ScopedObjectsNewDto dto, object source = null)
    {
