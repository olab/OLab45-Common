    ...
    public class XmlMediaElementsDto : XmlImportDto<XmlMediaElement>
    {
        ...
        public override bool Load(string importDirectory)
        {
            ...
            if (result)
            {
                try
                {
                    IEnumerable<dynamic> outerElements = (IEnumerable<dynamic>)GetXmlPhys().media_elements.media_elements_files.Elements();
                ...
                try
                {
                    IEnumerable<dynamic> outerElements = (IEnumerable<dynamic>)GetXmlPhys().media_elements.media_elements_avatars.Elements();
        ...
        public override bool Save(int recordIndex, IEnumerable<dynamic> elements)
        {
            ...
            try
            {
                string sourceDirectory = Path.Combine(GetImportPackageDirectory(), "media");

                XmlMapDto mapDto = GetImporter().GetDto(DtoTypes.XmlMapDto) as XmlMapDto;
                Model.Maps map = mapDto.GetModel().Data.FirstOrDefault();

                string targetDirectory = GetPublicFileDirectory("Maps", map.Id);
                Directory.CreateDirectory(targetDirectory);

                foreach (dynamic element in elements)
                {
                    dynamic fileName = Conversions.Base64Decode(element.Value);
