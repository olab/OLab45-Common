...
namespace OLabWebAPI.Common
{
  public abstract class WikiTag1Argument : WikiTagModule
  {
    ...
    {
      string source = GetWiki()[(GetWiki().IndexOf(':') + 1)..].Replace("]]", "");
      foreach (string pattern in wikiTagNamePatterns)
      {
    ...
    protected override string BuildWikiTagHTMLElement()
    {
      ...
      List<string> classes = new List<string> { GetHtmlElementName().ToLower(), wikiTagIdPart.ToLower().Replace(" ", "-") };
    ...
    public override string Translate(string source)
    {
      ...
      {
        string name = GetWikiArgument1();
        if (string.IsNullOrEmpty(name))
        {
          string str = $"**Invalid wiki tag {GetWiki()}**";
